...
import { Activity, formatPace, convertMovingTime2Sec } from '@/utils/utils';
...
const ActivityCard: React.FC<ActivityCardProps> = ({
  ...
  const formatLocalPace = (speed: number): string => {
  ...
  return (
    ...
    >
      <div className={`${styles.cardInner} ${isFlipped ? styles.flipped : ''}`}>
        ...
        <div className={styles.cardFront}>
          ...
          <div className={styles.activityDetails}>
            ...
            <p>
              ...
              {formatLocalPace(summary.averageSpeed)}
            </p>
            <p>
            ...
            {interval !== 'day' && (
              <>
                ...
                <p>
                  ...
                  {formatLocalPace(summary.maxSpeed)}
                </p>
              </>
            )}
        ...
        {/* Back side - Per-activity detail cards */}
        {interval === 'day' && activities.length > 0 && (
          <div className={styles.cardBack}>
            <div className={styles.dayActivitiesBack}>
              {activities.map((act) => {
                const distKm = (act.distance / 1000).toFixed(2);
                const pace = act.average_speed ? formatPace(act.average_speed) : '—';
                const hr = act.average_heartrate
                  ? `${act.average_heartrate.toFixed(0)} bpm`
                  : null;
                const elev =
                  SHOW_ELEVATION_GAIN && act.elevation_gain != null
                    ? `↑ ${act.elevation_gain.toFixed(0)}m`
                    : null;
                const movingSecs = convertMovingTime2Sec(act.moving_time);
                const hh = Math.floor(movingSecs / 3600);
                const mm = Math.floor((movingSecs % 3600) / 60);
                const ss = movingSecs % 60;
                const timeStr =
                  hh > 0
                ...
                const typeEmoji =
                  act.type === 'Run'
                    ? '🏃'
                    : act.type === 'cycling' || act.type === 'Ride'
                    ? '🚴'
                    : act.type === 'hiking'
                    ? '🥾'
                    : act.type === 'walking' || act.type === 'Walk'
                    ? '🚶'
                    : act.type === 'swimming'
                    ? '🏊'
                    : '🏅';
                return (
                  <div key={act.run_id} className={styles.dayActivityCard}>
                    <div className={styles.dayActivityHeader}>
                      <span className={styles.dayActivityEmoji}>{typeEmoji}</span>
                      <span className={styles.dayActivityName}>
                        {act.name || act.type}
                      </span>
                    </div>
                    <div className={styles.dayActivityStats}>
                      <div className={styles.dayStatItem}>
                        <span className={styles.dayStatValue}>{distKm}</span>
                        <span className={styles.dayStatLabel}>km</span>
                      </div>
                      <div className={styles.dayStatItem}>
                        <span className={styles.dayStatValue}>{pace}</span>
                        <span className={styles.dayStatLabel}>/km</span>
                      </div>
                      <div className={styles.dayStatItem}>
                        <span className={styles.dayStatValue}>{timeStr}</span>
                        <span className={styles.dayStatLabel}>time</span>
                      </div>
                      {hr && (
                        <div className={styles.dayStatItem}>
                          <span className={styles.dayStatValue}>{hr}</span>
                          <span className={styles.dayStatLabel}>avg hr</span>
                        </div>
                      )}
                      {elev && (
                        <div className={styles.dayStatItem}>
                          <span className={styles.dayStatValue}>{elev}</span>
                          <span className={styles.dayStatLabel}>elev</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <p className={styles.tapHint}>tap to go back</p>
            </div>
          </div>
        )}
      </div>
